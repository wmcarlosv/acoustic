...
<head>
    @php
        $app_name = \App\Models\Setting::first()->app_name;
        ...
        <section class="section">
            <div class="d-flex flex-wrap align-items-stretch">
                <div class="col-lg-4 col-md-6 col-12 order-lg-1 min-vh-100 order-2 bg-white">
                    <div class="p-4 m-3">
                        <img src="{{ asset('image/app/'.$color_logo) }}" alt="logo" width="80" class="shadow-light rounded-circle mb-5 mt-2">
                        <h4 class="text-dark font-weight-normal">{{__('Welcome to')}} <span class="font-weight-bold">{{$app_name}}</span></h4>
                        <p class="text-muted"> {{__("Before you get started, you must login or register if you don't already have an account.")}} </p>
                        <form method="POST" action="{{url('/admin/login/check')}}" class="needs-validation" novalidate="">
                            @csrf
                            ...
                            {{-- <div class="mt-5 text-center">
                                {{__("Don't have an account?")}} <a href="auth-register.html"> {{__('Create new one')}} </a>
                            </div> --}}
                        </form>

                        <div class="text-center mt-5 text-small">
                            {{__('Copyright')}} &copy; {{$app_name}}. {{__('Made with')}} 💙 {{__('by')}} SaaSMonks
                            {{-- <div class="mt-2">
                ...
                <div class="col-lg-8 col-12 order-lg-2 order-1 min-vh-100 background-walk-y position-relative overlay-gradient-bottom" data-background="{{ asset('image/admin_img/login-bg.jpg') }}">
                    <div class="absolute-bottom-left index-2">
                        <div class="text-light p-5 pb-2">
                            <div class="mb-5 pb-3">
                                <h1 class="mb-2 display-4 font-weight-bold"> {{__('Welcome!')}} </h1>
                                {{-- <h5 class="font-weight-normal text-muted-transparent"> {{$app_name}} </h5> --}}
                            </div>
                        </div>
                    </div>
                </div>
